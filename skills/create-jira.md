...
name: create-jira
description: >
  Use when 需要按团队风格撰写并创建 Jira（新建需求、子任务、操作项、可用性/平台化改进），
  或用户提到「创建 Jira」「建个票」「按我们风格写 Jira」「写个可执行 Jira」、
  「给我一版可直接贴 / 发到 Jira 的描述」，或要把调查结论/方案从需求里拿掉、按问题+待办重写，
...
把模糊需求写成可执行 Jira，并按确认结果创建 issue；保持团队当前风格一致，减少来回澄清。

**需求暴露问题和待办，不携带已查到的结论和做事方式。**

## 何时使用

- 用户说「创建 Jira」「建个票」「按我们风格写 Jira」「帮我写一个任务」
- 只有一句背景，需要整理成可落地的 issue 并（可选）创建
- 需要统一 `Usability:` / `Operation:` / `Scalability:` 开头与描述结构
- 已有调查/脚本/数字，但用户要的是需求，不是结论文档
- 需要决定任务 vs 子任务、父项、Relates

## 流程概览

1. **写**：按下方固定写法整理 Summary / Description
2. **确认层级**：要建 Subtask 则先问父项 key；未确认 → 独立任务
3. **创建**：用户要建票时再 POST；只写文案则输出可贴内容即可
...
1. **先确认再创建 Subtask。** 要建成 Subtask 时，必须先问用户挂在哪条下面，得到明确 issue key 后再 POST。不要猜 `11539` / 当前 Q3 故事 / Relates 对象的父项
2. 没确认 → 默认建成独立「任务」，无 parent
3. 用户说关联到某条（如 `11874`）→ 只 Relates，**不要**顺便挂它的父项
4. 双挂会乱：同一条既是 11539 的子任务又 Relates 到 11539 的另一条子任务
5. Jira **调不了**：Subtask 不能改成任务、也不能换父项类型。挂错只能新建 + 删旧（换 key）。所以父项只能问，不能先挂再改
6. 用户只要文案、不说创建 → 只输出可贴内容，不 POST
